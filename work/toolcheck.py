...
TOOLMAP = {
    ...
    # does `LOAD 'Include.Symbols'` — the MPW symbol-dump generated by the
    # makefile rule `AsmIIGS -c GenerateDump`; asm_opts.loads mirrors that
    # rule (native replay, see asm.do_load / tests/fixtures/054).
    # DebugSymbols=0 is the .aii.obj build rule's -d define (release build).
    '034': ('TextEdit',   {'files': [
        'highlevel.aii', 'block.aii', 'defproc.aii', 'draw.aii', 'entry.aii',
        'fastdraw.aii', 'format.aii', 'idle.aii', 'key.aii', 'measure.aii',
        'memory.aii', 'record.aii', 'scrap.aii', 'scroll.aii',
        'selection.aii', 'super.aii', 'text.aii', 'width.aii', 'wrap.aii'],
        'asm_opts': {'defines': {'DebugSymbols': 0},
                     'loads': {'Include.Symbols': 'GenerateDump'}}}),
}

# Work-in-progress mappings: run individually (`toolcheck.py NNN`) and shown
# in the summary, but EXCLUDED from the CORPUS tally the gate consumes — a
# non-exact tool must not enter the gate denominator (E3 house rule: mapping
# is gate-visible only once byte-exact).  Empty since Tool034 closed
# (2026-07-19: 35651/35651).
WIP = set()
...
def link_module(roots, asm_opts=None):
    ...
    Returns the concatenated, relocated code image.  ``asm_opts`` passes
    per-tool assemble() kwargs (defines / loads) from the TOOLMAP entry."""
    objects = []
    for r in roots:
        a = asm.assemble(f'{TB}/{r}', INCS, **(asm_opts or {}))
        obj = omf.emit(a)
        objects.append((obj, a))
...
def check(tool, verbose=False):
    ...
    if isinstance(spec, dict):
        ...
        # flat link with per-tool assemble options
        asm_opts = spec.get('asm_opts')
        spec = spec['files']
    ...
    if g is None:
        return tool, subdir, None, "no golden binary"
    try:
        mine = link_module([f'{subdir}/{r}' for r in roots], asm_opts=asm_opts)
    except Exception as e:
        return tool, subdir, None, f"{type(e).__name__}: {e}"
...
def main():
    ...
    for t in sorted(TOOLMAP):
        ...
        wip = '  [WIP, excluded from tally]' if t in WIP else ''
        if res is None:
            print(f"Tool{t}  {sub:12} {'--':>7}  {err}{wip}")
            continue
        pct, m, n, lg, lo = res
        if t not in WIP:
            tot_m += m; tot_n += n
        print(f"Tool{t}  {sub:12} {pct:>6}%  {lg:>8}/{lo:<8}  ({m}/{n} bytes){wip}")
    if tot_n:
