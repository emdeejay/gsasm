...
Multi-segment tools (MenuMgr, ControlMgr) ship several load segments plus a
linker-generated `~JumpTable`; they are compared PER SEGMENT (see _check_multiseg
and the '015'/'016' TOOLMAP entries), each segment relocated from its own base 0,
exactly as the gold ExpressLoad file stores it. Flat-linking such a tool into one
blob and comparing against the concatenated segments is WRONG — it makes every
intra-segment reloc look "off by the segment base" (see work/tool016_diag.py:
Tool016's 451-byte "residual" was 100% that artifact; per-segment it is byte-exact
bar one `~JumpTable`-routed byte).

Known remaining residuals (per-module levers, not a shared blocker):
  * `~JumpTable` segments (Tool015/016/018) are generated by the MPW linker and
    gsasm does not emit them (docs/TODO.md §2); a reference to a DYNAMIC segment
...
TOOLMAP = {
    ...
    '016': ('ControlMgr', {'segments': [
        {'gold_name': 'main',     'srcs': ['ControlMgr.asm', 'SuperControl.asm',
