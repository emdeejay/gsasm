...
| All 10 mapped toolbox toolsets (WindMgr, MenuMgr, ControlMgr, PrintMgr, LineEdit, DialogMgr, Scrap, StdFile, FontMgr, ListMgr) | 124,160 bytes | `work/toolcheck.py` |
...
unchanged (gate at baseline, buildrom byte-identical):
  ...
  * **`subcmds` record label `next` masking a proc-local branch target** — the
    ...
    bind the field, not a proc-local `temp dc.w 0`). `work/fstcheck.py` still
    reports AppleShare informationally (out of the byte-exact CORPUS tally) until
    the gate `fst_bytes` baseline is bumped to fold it in. Tool015/016/018 embed `~JumpTable` segments generated by the
