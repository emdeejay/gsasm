...
- **GS.OS residual 8 bytes** (was 44) — gsasm assembler/linker bugs, not an
  external floor. FOUR classes CLOSED 2026-07-17:
  ...
  Remaining 8: (a) ~4 B init.4 field-offset immediates (`ldy #s_flags`/`#id`/
  `adc #entry_size` bake $00 vs golden $0e/$0c/$10 — record field names resolving
  to 0 instead of their template offsets; a `record`/`WITH` field-scoping bug,
  still to root-cause); (b) 3 B scm_main (immediate $255C baked $005C + a
  mis-scoped duplicate `MORE`, $F99B vs $B70A); (c) 1 B be0segr (`BANK_E0_SEGR+
  $A86` placed off by 2). Each a real fix in `asm.py`/`linkiigs.py` — gate-verify hard.
