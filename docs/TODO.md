...
(`work/archive/reloc_survey.py`) to literal addends:
...
- **The multi-segment ExpressLoad path (`multiseg=True`) never emits ANY
  ...
  **single-segment** path's standalone-reloc scan instead — EasyMount's
  ...
  `multiseg=True` at all: `work/archive/toolsetup_probe.py`'s Tool.Setup output is
...
Historical starting point: these were once classed as "generated by the MPW
linker, not present in source." Controlled link experiments reversed the
jump-table generation algorithm and closed all three mapped tools.

**Superseded intermediate state for Tool016 (2026-07-18).** Tool016's 451-byte
"residual" was fully root-caused to a segmentation/harness artifact —
`work/archive/tool016_diag.py` proves gsasm assembles ControlMgr byte-exact per segment
...
came down to **one byte** plus generated `~JumpTable` routing. That one byte
(`main` `0x1022`) was itself a `~JumpTable`-routed reference; the landed
generation below closes it.
...
`work/archive/jumptable_probe.py` (`JUMPTABLE_FORMAT 4 ok / 0 bad`). Tool016's is even
...
**DONE (2026-07-18) — Tool015/016/018/020 byte-exact; the mapped tool corpus is
now 150459/150459.** The linker-side
generation landed in `gsasm/expressload.py` (`encode_jumptable` / `jt_jsl_offset`,
ported from `work/archive/jumptable_probe.py`), a one-line opt in `gsasm/linkiigs.py`
...
Mechanics, all validated against gold:
  ...
  - **~JumpTable gated.** `_check_jt_tool` builds the KIND-2 segment via
    ...
    not in the per-segment `tool_bytes` denominator. Tool020 has NO dynamic segment (TheProc is
    KIND 0x4000, not 0x8000): its far pointer is a direct cINTERSEG, no jump table.

**Tool018 (QDAux) — CLOSED and mapped.** `_link_jt_tool` derives Tool018's
12-entry ~JumpTable byte-exact (ordering proven above), handles the copybits.asm
SEG-section split, links all five load segments, and closes the SeedFill byte.
Tool018 is now part of `toolcheck.py`'s mapped corpus at 26,299/26,299.

## 3. P8 — DONE (byte-exact)

P8 (`/System.Disk/System/P8`, 17128 bytes) is now **byte-exact**, gated by
`work/p8check.py` (`p8_bytes` 17128/17128). The "missing include files" framing
was wrong: every include resolves and `OverlayIIgs == makebin.overlay`. The real
gaps were assembler-dialect bugs, all fixed (fixtures 049–053):
- `MACHINE M6502/M65C02` forces `LONGA/LONGI OFF` (8-bit immediates: QuitCode, Ram);
- backward mid-segment `ORG` overlay in an ORG'd/absolute segment (`jmp/jsr 0 /
  org *-2` self-modified vectors — PROCONE/PROCTHREE);
- `IF (A) < (B)` conditional-paren evaluation (the loader-pad `if`, PROCTHREE);
- DS-count expression folding across blanks (Sel.Alt.n's paragraph pad);
- undefined top-level `&NAME` stays literal (CClock `&MIKE`);
- `;`-comments are not &-substituted (Ram `&BLOCK`).
The four MLI PROC segments link together via `makebin.makebin_segments` (a
combined symbol table, mirroring `linkiigs -lseg NAME -org $X`); the ram driver
segments likewise. `work/kernelcheck.py`'s old raw-PROCONE line is now
informational only (it compared PROCONE against the overlaid golden).
...
- **AppleShare.FST → byte-exact** — MOSTLY DONE (2026-07-17): 30% → 99.9%
  ...
  baseline and corpus-free fixtures:
    ...
    4. Harness encoding (`work/fstcheck.py::_build_appleshare`, and
       `work/archive/appleshare_diag.py`): the rewritten temp modules were written UTF-8
  ...
  See `work/archive/appleshare_diag.py` (maps any divergence to source). The MakeFile
...
- **P8 (§3)** — DONE, byte-exact (`work/p8check.py`, 17128/17128); the "missing
  includes" framing was wrong (assembler-dialect bugs, all fixed).
