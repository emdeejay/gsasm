...
# ---------------------------------------------------------------------------
# The ExpressLoad ``~JumpTable`` segment is generated by MPW LinkIIgs for a
# multi-segment load file: each far pointer from a resident segment into a
# DYNAMIC (KIND & 0x8000, on-demand) segment is routed through a 14-byte thunk
# here, so the first call traps into the GS/OS System Loader's ``JumpLoad``
# (loads/relocates the target segment, overwrites the thunk's JSL with a `$5C`
...
#                 JE_UserID  (2) = 0      patched to the real UserID at load
#                 JE_FileNum (2) = 1      this load file
#                 JE_SegNum  (2) = target dynamic segment number (1-based file order)
#                 JE_Offset  (4) = routine offset within that segment
#                 JE_jsl     (4) = 22 00 00 00   $22=JSL, addr patched to JumpLoad
#     trailer : 4 bytes of 0x00
#     total   = 8 + 14*N + 4
#
# A far pointer to entry i's routine relocates to
# ``~JumpTable + (SEG_JMP_START + i*JMP_ENTRY_SIZE + JE_JSL)`` — the entry's JSL.

SEG_JMP_START = 8      # Loader.Equates seg_jmp_start (header length)
JMP_ENTRY_SIZE = 14    # Loader.Equates jmp_entry_size
JE_JSL = 10            # Loader.Equates JE_jsl (JSL offset within an entry)


def jt_jsl_offset(entry_index: int) -> int:
    """Segment-relative offset of jump-table entry *entry_index*'s JSL — the
    far-pointer target a reference to that entry's dynamic routine relocates to."""
    return SEG_JMP_START + entry_index * JMP_ENTRY_SIZE + JE_JSL


def encode_jumptable(entries: list[tuple[int, int]], file_num: int = 1) -> bytes:
    """Encode ``[(target_seg_num, routine_offset), ...]`` as the ~JumpTable
    LCONST image (the 26/180/... byte KIND-2 segment body).  ``entries`` order
    is the reference-scan order — an entry's index fixes the JSL offset every
    caller of that routine is rewritten to (see ``jt_jsl_offset``)."""
    ...
    for seg_num, offset in entries:
        ...
        out += b'\x22\x00\x00\x00'                                # JSL opcode + unpatched addr
